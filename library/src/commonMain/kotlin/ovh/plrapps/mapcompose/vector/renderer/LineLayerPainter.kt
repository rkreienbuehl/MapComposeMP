...
class LineLayerPainter(
    ...
    override suspend fun paint(
        ...
        val lineBlur = paint.lineBlur.processAsFloat(featureProperties, actualZoom) ?: 0f
        val lineOffset = paint.lineOffset.processAsFloat(featureProperties, actualZoom) ?: 0f
        val lineTranslate =
            paint.lineTranslate.processAsDoubleList(featureProperties, actualZoom)?.map { it.toFloat() } ?: listOf(0f, 0f)
        val lineTranslateAnchor = paint.lineTranslateAnchor?.processAsString(featureProperties, actualZoom) ?: "map"
        val lineCap = getLineCap(layout, actualZoom, featureProperties)
        val lineJoin = getLineJoin(layout, actualZoom, featureProperties)
        val lineMiterLimit = layout?.lineMiterLimit.processAsFloat(featureProperties, actualZoom) ?: 2f
        val lineRoundLimit = layout?.lineRoundLimit.processAsDouble(featureProperties, actualZoom) ?: 1.0
